...
}
{
    ...
    do
    {
        Console.WriteLine("Input command (add/move/stop)");
        switch (ConsoleInput.MEAN_PART.check(Checks.Into(["add", "move", "stop"]), "Command must be add, move or stop").get())
        {
            case "add":
                ...
                break;
            case "move":
                Console.WriteLine("Input name to move");
                Game? deleted = null;
                string name = ConsoleInput.MEAN_PART.get();
                if (linkList.PopLast(game => game.Name == name, ref deleted)) linkList.PushLast([deleted]);
